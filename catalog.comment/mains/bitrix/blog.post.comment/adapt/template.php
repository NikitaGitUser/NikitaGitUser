				...
				<div class="blog-comment-fields">
					...
					echo '
						<div class="blog-comment-field-text">
							<label for="message">'.GetMessage('MESSAGE_INPUT').'</label>
...
//					only for not registered users
					...
					<div class="blog-comment-buttons-wrapper">
						<input tabindex="10"class="btn btn-default" value="<?=GetMessage("B_B_MS_SEND")?>" type="button" name="sub-post" id="post-button" onclick="submitCommentNew()">
					</div>
				</div>
		...
		function ShowComment($comment, $tabCount=0, $tabSize=2.5, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
		{
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div id="record-<?=$arParams["ENTITY_XML_ID"]?>-<?=$comment["ID"]?>-placeholder" class="blog-comment-form" style="display:none;"></div>
...
//							dbg showComment?
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment"><a class="btn btn-default white" href="javascript:void(0)" onclick="return editCommentNew('0', <?=$arParams["ID"]?>)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a><br /></div>
				<a name="0"></a>
				<?
			...
			if($arResult["CanUserComment"])
			{
				...
				<div id="form_comment_0">
					...
					<!--				placeholder for past editor					-->
					<div id="record-<?=$arParams["ENTITY_XML_ID"]?>-0-placeholder" class="blog-comment-edit feed-com-add-block blog-post-edit blog-comment-form" style="display:none;"></div>
